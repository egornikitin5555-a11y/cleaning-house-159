...
// ============================================
// МОБИЛЬНОЕ ВИДЕО - ОДНОКРАТНОЕ ВОСПРОИЗВЕДЕНИЕ
// ============================================

/**
 * На мобильных устройствах: видео проигрывается один раз,
 * затем плавно появляется картинка с логотипом
 */
function initMobileVideoOnce() {
    const heroVideo = document.getElementById('heroVideo');
    const videoPoster = document.getElementById('heroVideoPoster');
    
    if (!heroVideo || !videoPoster) return;
    
    // Определяем мобильное устройство (ширина экрана <= 768px)
    const isMobile = window.matchMedia('(max-width: 768px)').matches;
    
    if (isMobile) {
        // Убираем loop для мобильных
        heroVideo.removeAttribute('loop');
        heroVideo.loop = false;
        
        let posterShown = false;
        
        // Функция показа постера
        function showPoster() {
            if (posterShown) return;
            posterShown = true;
        ...
        heroVideo.addEventListener('ended', showPoster, { once: true });
        
        // Резервный механизм: если видео не воспроизводится (autoplay заблокирован)
        // показываем постер через 2 секунды
        setTimeout(function() {
            if (heroVideo.paused && !posterShown) {
                console.log('Video paused - showing poster');
                showPoster();
            }
        }, 2000);
        
        // Также проверяем при ошибке загрузки видео
        heroVideo.addEventListener('error', function() {
            console.log('Video error - showing poster');
            showPoster();
        }, { once: true });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // -------------------------------------------
    // 0. МОБИЛЬНОЕ ВИДЕО (однократное воспроизведение)
    // -------------------------------------------
    initMobileVideoOnce();
